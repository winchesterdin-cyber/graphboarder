...
<script lang="ts">
	...
	function handleEndpointClick(endpoint: any) {
		const prefix =
			endpoint.id.startsWith('localEndpoint--') || endpoint.id.startsWith('localstorageEndpoint--')
				? '' // already has prefix
				: customEndpoints.find((e: any) => e.id === endpoint.id)
	...
	>
		...
		<div class="tabs tabs-boxed mb-4">
			<button
				...
				onclick={() => (activeTab = 'list')}>List</button
			>
			<button
				class="tab {activeTab === 'add' ? 'tab-active' : ''}"
				onclick={() => (activeTab = 'add')}>Add New</button
			>
		</div>
		...
		{#if activeTab === 'list'}
			<div class="overflow-y-auto max-h-[60vh] space-y-2 p-1">
				...
				{#each allEndpoints as endpoint}
					<div
						class="flex items-center justify-between bg-base-200 p-3 rounded hover:bg-base-300 transition-colors"
					>
						<button class="flex-1 text-left" onclick={() => handleEndpointClick(endpoint)}>
		...
		{:else}
			<div class="space-y-4">
				...
				<div class="flex justify-end gap-2 mt-4">
					...
					<button class="btn btn-primary" onclick={addEndpoint} disabled={!newName || !newUrl}
						>Add Endpoint</button
					>
				</div>
			</div>
